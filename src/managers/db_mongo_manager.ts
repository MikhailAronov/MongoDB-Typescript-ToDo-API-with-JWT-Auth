...
async function testDataBAZAandQueries (db : any) {
    ...
    await db.ToDo.bulkWrite(todosUpdateQuery);
...
async function createToDos(db : any) {
...
async function showAllCollections(db : any) {
    console.log('Users collection : ', await db.Users.find({}));
    console.log('Keys collection  : ', await db.Keys.find({}));
    console.log('ToDo collection  : ', await db.ToDo.find({}));
}

async function cleanAll(db : any) {
...
async function mongo() : Promise<void> {
    try {
        const uri : string = 'mongodb+srv://IvIjDBr:LLvFyrt@womanupmongo.bepahne.mongodb.net/?retryWrites=true&w=majority';
